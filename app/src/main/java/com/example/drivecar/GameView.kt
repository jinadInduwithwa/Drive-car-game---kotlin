...
class GameView (var c:Context, var gameTask: GameTask):View(c){
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        d.setBounds(
            myCarPossition * viewWidth / 3 + viewWidth / 15 +25,
            viewHeight-2 - carHeight,
            myCarPossition * viewWidth / 3 + viewWidth / 15 + carWidth - 25,
            viewHeight - 2
        )
        d.draw(canvas!!)
        ...
        for(i in otherCars.indices){
            try{
                ...
                val d2 = resources.getDrawable(R.drawable.redcar,null)

                d2.setBounds(carX+25,carY - carHeight, carX + carWidth - 25, carY)
                d2.draw(canvas)
                if(otherCars[i]["lane"] as Int == myCarPossition){
